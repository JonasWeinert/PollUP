...
export function CreateElementModal({ sessionId, onClose }: CreateElementModalProps) {
  const [type, setType] = useState<ElementType>("single_choice");
      ...
      if (type === "single_choice" || type === "single_choice_unique" || type === "multiple_choice") {
        choicesData = await Promise.all(
          choices.map(async (choice) => {
            let choiceImageId: Id<"_storage"> | undefined;
            if (choice.imageFile) {
    ...
    <div className="fixed inset-0 bg-black/70 z-[60] flex items-center justify-end" onClick={onClose}>
      ...
      >
        <div className="sticky top-0 bg-white/95 backdrop-blur-sm border-b border-gray-200 px-8 py-5 flex items-center justify-between z-10">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 tracking-tight">Create New Element</h2>
            <p className="text-sm text-gray-500 mt-0.5">Add interactive content to your session</p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            ...
            <select
              ...
              className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all bg-gray-50 hover:bg-white text-base"
            >
              <option value="single_choice">Single Choice</option>
              <option value="single_choice_unique">Single Choice - Unique Answers (each option can only be selected once)</option>
              <option value="multiple_choice">Multiple Choice</option>
              <option value="text_input">Text Input</option>
              <option value="number_input">Number Input (Slider)</option>

...
export function EditSessionModal({ session, onClose }: EditSessionModalProps) {
  ...
  const [resultsPublic, setResultsPublic] = useState(session.resultsPublic);
  const [resultsPinCode, setResultsPinCode] = useState(session.resultsPinCode || "");
  const [completionTitle, setCompletionTitle] = useState(session.completionTitle || "");
  const [completionSubtitle, setCompletionSubtitle] = useState(session.completionSubtitle || "");
  const [completionDescription, setCompletionDescription] = useState(session.completionDescription || "");
  const [completionImageFile, setCompletionImageFile] = useState<File | null>(null);
  const [bgColor, setBgColor] = useState(session.bgColor || "#f9fafb");
  const [accentColor, setAccentColor] = useState(session.accentColor || "#2563eb");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateSession({
        ...
        completionImageId: completionImageFile ? completionImageId : undefined,
    ...
    <div className="fixed inset-0 bg-black/70 z-[60] flex items-center justify-end" onClick={onClose}>
      ...
      >
        <div className="sticky top-0 bg-white/95 backdrop-blur-sm border-b border-gray-200 px-6 sm:px-8 py-5 flex items-center justify-between z-10">
          <div>
            <h2 className="text-xl sm:text-2xl font-bold text-gray-900 tracking-tight">Edit Session</h2>
            <p className="text-xs sm:text-sm text-gray-500 mt-0.5">Update your classroom experience settings</p>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="border-t border-gray-200 pt-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                />
                {session.completionImageId && !completionImageFile && (
